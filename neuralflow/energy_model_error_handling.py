...
def check_em_parameters(func):
    ...
    def check_and_set_init_vars(self_var,num_neuron=1,
        ...
        if not np.issubdtype(type(num_neuron),np.integer) or num_neuron <1:
            raise ValueError("number of neurons: not integer of less than 1")
            ...
            if callable(fr):
                ...
                if len(fr['params'].values())>0:
                    ...
                    for iFun in range(n_neurons):
                        self_var.firing_model_.append(lambda x, i=i, iFun=iFun, params_list=params_list: self_var._firing_model_types[fr['model']](x, **params_list[iFun])) 
                else:
                    self_var.firing_model_.append(lambda x, i=i: self_var._firing_model_types[fr['model']](x))
            else:
                raise ValueError("Firing rate model #{}: unknown or not provided".format(i))          
        ...
        if callable(peq_model):
            ...
            if peq_model['model'] in self_var._peq_model_types:
                self_var.peq_model_ = lambda x, w: self_var._peq_model_types[peq_model['model']](x, w, **peq_model['params'])
            else:
        ...
        #Check Nv:
        if (not np.issubdtype(type(Nv),np.integer) or num_neuron <1) and Nv is not None:
            raise ValueError("Nv: not integer of less than 1")
        
        # Force verbose type to bool
        verbose = bool(verbose)
        
        func(self_var,num_neuron,firing_model,peq_model,p0_model,boundary_mode,D0,Nv,pde_solve_param,verbose)
...
def _check_optimization_options(self, optimizer, options):
    ...
    optimization_options=['gd_type','gamma','max_iteration','loglik_tol','etaf']
    ...
    if save is not None:
        ...
        if 'schedule' in save and save['schedule'] is not None:
            ...
            if save['schedule'][-1]<max_iteration+save['sim_start']+new_sim-1:
                if self.verbose:
                    print("Setting max_iteration to {}".format(save['schedule'][-1]-save['sim_start']+1-new_sim))
                optimization['max_iteration']=int(save['schedule'][-1]-save['sim_start']+1-new_sim)
            ...
            if self.verbose:
                print('Warning: schedule is not provided. The results will be saved on every iteration')
            options['save']['schedule']=np.array(range(save['sim_start'],save['sim_start']+max_iteration+1)).astype(np.int)
    ...
    if 'gamma' not in optimization:
        raise ValueError('gamma must be specified in options for {} optimizier for at least one parameter'.format(optimizer))
        ...
        if not all ((isinstance(values, numbers.Number) or isinstance(values, list) or values>0) and keys in self._parameters_to_optimize for keys,values in optimization['gamma'].items()):
            raise ValueError ('Error in gamma: unsupported parameter or learning rate not a number')
