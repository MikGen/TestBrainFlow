...
class EnergyModel(BaseEstimator):
    ...
    p0_model : dictionary or callable or None
        ...
        ``peq_models.py`` file. If set to None, it will be assumed to be equal to peq (the equilibirum case).
        The default is None.
    D0 : float
    ...
    Nv : int
        ...
        result in large computational times. The default value is N-2, where N is the total number of grid points(for SEM N=Ne*(Np-1)+1).
    ...
    def __init__(self, num_neuron, firing_model, peq_model, p0_model, boundary_mode, D0, Nv, pde_solve_param, verbose):
        ...
        if boundary_mode is not None:
            bcs={"reflecting":"Neumann","absorbing":"Dirichlet"}[boundary_mode]
