...
def add_anchor_point(x,y):
    ...
    not_found=True
    cpoly=PPoly(CubicSpline(np.sort(x[1:3]), np.sort(y[1:3]), bc_type=[(1, 0), (1, 0)]).c,np.sort([x[1],x[2]]))
    ...
    while not_found:
       ...
       if first_peak_is_maximum:
           if cpoly(x_add_mirror) > y[0] and np.abs(x_add_mirror-x[1])<np.abs(x[1]-x[0]):
               ...
               if x[1]>x[0]:
                   poly2=PPoly(CubicSpline([x[0], x_add, x[1]], [y[0], cpoly(x_add_mirror), y[1] ], bc_type=[(1, 0), (1, 0)]).c,[x[0],x_add,x[1]])
               else:
                   poly2=PPoly(CubicSpline([x[1], x_add, x[0]], [y[1], cpoly(x_add_mirror), y[0] ], bc_type=[(1, 0), (1, 0)]).c,[x[1],x_add,x[0]])
