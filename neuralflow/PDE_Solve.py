...
class PDESolve:
    ...
    BoundCond : dictionary
        ...
        # change in periodic BC branch(09/05/21): ---------------------------------------------------------------------------------------------
        The default in the master branch is {'leftB': 'Neumann', 'rightB': 'Neumann'}
        Default here is {'coupled':'periodic'}: coupled is just to suggest that the left and right boundary are considered in one equation.
        # -------------------------------------------------------------------------------------------------------------------------------------
    ...
    def __init__(self, xbegin=-1.0, xend=1.0, method={'name': 'SEM', 'gridsize': {'Np': 8, 'Ne': 256}},
                 BoundCond={'coupled':'periodic'}, grid_mode_calc ='newton', 
    ...
    def Integrate(self,f,result=None):
        ...
        if result is None:
            ...
            if self.int_mode == 'full':
                ...
                self.AD_d.dot(f,out=result)
                result+=np.append([0],np.repeat(np.cumsum(result[0:-1:self.Np-1]),self.Np-1))
    ...
    def solve_EV(self, peq=None, D=1, q=None, w=None, mode='hdark', fr=None, Nv=64):
        ...
        if self.method['name']=='FD':
            lQ, QxOrig = linalg.eigh(self.stiffmat_full_.A, self.massmat_full_.A, eigvals=(0, Nv - 1))
        elif self.method['name']=='SEM':
            lQ, QxOrig = linalg.eigh(self.stiffmat_red_.A, self.massmat_red_.A, eigvals=(0, Nv - 1)) 
        ...
        if self.method['name']=='FD': 
            ...
            if self.BC_method=='bound_subst': 
                if self.BC_[1]==0: 
                    QxOrig = np.concatenate((np.reshape(np.zeros(Nv),(1,Nv)),QxOrig),axis=0)
                if self.BC_[3]==0:
                    QxOrig = np.concatenate((QxOrig,np.reshape(np.zeros(Nv),(1,Nv))),axis=0)
        ...
        if mode=='hdark':
            ...
            assert(all(lQd[i] <= lQd[i+1] for i in range(len(lQd)-1))), 'Error! Returned EVVd not sorted'
    ...
    def _setmat(self,p,q,w):
        ...
        if self.method['name']=='FD':
            ...
            c1=self.BC_[1]/(self.BC_[1]-self.BC_[0]*self.dx) 
            c2=self.BC_[3]/(self.BC_[3]+self.BC_[2]*self.dx) 
            self.stiffmat_full_[0,0]+=c1*p_bp[0] # Take care of BC[xbegin]
            self.stiffmat_full_[-1,-1]+=c2*p_bp[-1] # Take care of BC[xend]
            ...
            if self.BC_method=='bound_subst':
                ...
                #Take care of boundary conditions:
                if self.BC_[1]!=0: 
                    self.stiffmat_full_[0,0]+=p[0]*self.BC_[0]/self.BC_[1]
                if self.BC_[3] != 0: 
                    self.stiffmat_full_[-1,-1]-=p[self.N-1]*self.BC_[2]/self.BC_[3]
                
                #Reduce matrix sizes if needed
                idx_s, idx_e = (self.BC_[1]==0),self.N-(self.BC_[3] == 0)
                self.stiffmat_red_=self.stiffmat_full_[idx_s:idx_e,idx_s:idx_e]
                self.massmat_red_=sparse.diags(self.w_d[idx_s:idx_e]*w[idx_s:idx_e], 0)
                ...
                self.massmat_red_ = self.NullM_.T.dot(self.massmat_full_.dot(self.NullM_))
                self.stiffmat_red_ = self.NullM_.T.dot(self.stiffmat_full_.dot(self.NullM_))
    ...
    def _get_grid(self):
        ...
        if self.method['name']=='FD': 
            ...
            for i in range(self.Ne):
                patch=np.arange(i * (self.Np - 1),i * (self.Np - 1) + self.Np)
    ...
    def _get_single_element(self):
        ...
        self.x_ = np.append(np.append(-1,legendre.Legendre(coefs).deriv().roots()),1)
        ...
        for i in range(self.Np):
            for j in range(self.Np):
                if i != j:
                    self.dmat_[i][j] = Ln[i] / (Ln[j] * (self.x_[i] - self.x_[j]))
                else:
                    self.dmat_[i][i] = 0
    ...
    def _get_single_element_numerics(self):
        ...
        while max(abs(self.x_ - xold)) > MACHINE_EPSILON:
            ...
            self.x_ = xold - (self.x_ * P[:, self.Np-1] - P[:, self.Np - 2]) / (self.Np * P[:, self.Np-1])
        ...
        for i in range(self.Np):
            for j in range(self.Np):
                if i != j:
                    self.dmat_[i][j] = Pend[i] / (Pend[j] * (self.x_[i] - self.x_[j]))
                else:
                    self.dmat_[i][i] = 0
    ...
    def _get_BC(self):
        ...
        # the len = 4 BC_ vector is not used in periodic BC --------------------------------------------------------------------
    ...
    def _get_Nullspace(self):
        ...
        if self.BC_method=='projection':
            ...
            if 'coupled' in  self.BoundCond.keys():
                # here we only have periodic, p(1) = p(-1), and p'(1) = p'(-1):
                if self.BoundCond['coupled'] == 'periodic':
                    BCmat=np.zeros((2,self.N))
                    # BCmat[0, :] should be like [1, 0, ..., 0. -1]
                    BCmat[0,0]= 1
                    BCmat[0,-1]= -1
                    # dmat_d is a sparse matrix, this represents the differential operator on the end points:
    ...
    def _set_AD_mat(self):
        ...
        x_mat=(np.transpose(np.matlib.repmat(x_local,self.Np,1)))**np.arange(1,self.Np+1)
        for i in range(self.Np):
            ...
            Combinations=[sum(reduce(mul, c) for c in combinations(x_crop, i + 1)) for i in range(self.Np-1)]
            coefs[:-1]=((-1)**np.arange(1-self.Np%2,self.Np-self.Np%2))*Combinations[::-1]/np.arange(1,self.Np)
        ...
        if self.int_mode=='full':
            for i in range(self.Ne):
                ...
                self.AD_d[np.ix_(np.arange(i * (self.Np - 1)+self.Np,self.N),patch)]+=self.AD_[-1,:]
        elif self.int_mode=='sparse':
            for i in range(self.Ne):
                patch=np.arange(i * (self.Np - 1),i * (self.Np - 1) + self.Np)
                self.AD_d[np.ix_(patch,patch)] += self.AD_
    ...
    def _get_matrices(self):
        ...
        if self.method['name']=='FD':
            self.stiffmat_full_=sparse.lil_matrix((self.N-2,self.N-2),dtype=np.float64)
            self.massmat_full_=sparse.lil_matrix((self.N-2,self.N-2),dtype=np.float64)
        
        elif self.method['name']=='SEM':
            
            #Full matrices are of size NxN
            self.stiffmat_full_=sparse.lil_matrix((self.N,self.N),dtype=np.float64)
            self.massmat_full_=sparse.lil_matrix((self.N,self.N),dtype=np.float64)
            ...
            if self.BC_method=='bound_subst':
                self.stiffmat_red_=sparse.csr_matrix((idx_e-idx_s,idx_e-idx_s),dtype=np.float64)
                self.massmat_red_=sparse.csr_matrix((idx_e-idx_s,idx_e-idx_s),dtype=np.float64)
            elif self.BC_method=='projection':
                self.stiffmat_red_=sparse.csr_matrix((self.N-2,self.N-2),dtype=np.float64)
                self.massmat_red_=sparse.csr_matrix((self.N-2,self.N-2),dtype=np.float64)
    ...
    def _check_and_set_params(self):
        ...
        assert (self.grid_mode_calc in {'built_in','newton'}), 'Unknown grid_mode_clc'
        assert (self.int_mode in {'full','sparse'}), 'Unknown integration mode'
        if self.method['name']=='FD':
            self.N=self.method['gridsize']['N']
            assert(self.N>0), 'N is incorrect'
        elif self.method['name']=='SEM':
            self.N=self.method['gridsize']['Ne']*(self.method['gridsize']['Np']-1)+1
            self.Np=self.method['gridsize']['Np']
            self.Ne=self.method['gridsize']['Ne']
            assert (self.Np>0 and self.Ne>0 and self.N>0), 'Total number of grid points incorrect'
        if isinstance(self.BoundCond,np.ndarray):
        ...
        # add Periodic Param Requriements --------------------------------------------------------------------------------
        else:
