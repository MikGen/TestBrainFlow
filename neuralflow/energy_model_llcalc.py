...
def _get_loglik_data(self, data, metadata=None, peq=None, rho0=None, D=None, fr=None, mode='loglik', grad_list=None, EV_solution=None):
    ...
    grad_list : list
        List of parameters for which gradients should be calculated (only for the 'gradient' mode). Supported options are 'F', 'F0', and 'D' . The default is None.  
    ...
    for iSeq in range(num_seq):
        ...
        last_event_is_spike=metadata["last_event_is_spike"][iSeq] if metadata["last_event_is_spike"][iSeq] is not None else True
        absorption_event=metadata["absorption_event"][iSeq] if metadata["absorption_event"][iSeq] is not None else False    
...
def _get_loglik_seq(self, data, peq, rho0, D, fr, mode, grad_list, EV_solution,last_event_is_spike,absorption_event):
    ...
    for i in reversed(range(S_Total)):
        ...
        #G function
        c_get_gamma.getGamma0(G, self.Nv, EV_solution["lQd"], tempExp, alpha, btemp, dt, i)
    ...
    if 'F0' in grad_list:
        results['F0']=-self.Integrate(rho0*np.sqrt(peq)-rho0*Qxd.dot(btemp/anorm[0]))
