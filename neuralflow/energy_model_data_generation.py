...
def generate_data(self, deltaT=0.00001, time_epoch = [(0,1)], decision_bnd=None,last_event_is_spike=True):
    ...
    time_epoch : list 
        List of N tuples, where N is the number of trials. Each tuple consists of start time and stop time in seconds. For the case of absorbing boundary, stop time will be the maximum allowed time 
        for the trial to last (the trial will terminate before this time due to absorption, or at this time in an arbitrary latent state). The default is [(0,1)].
    decision_bnd : list (N,).
        List that consists of N tuples, each of which consists of the positions of left and right boundaries in the latent space. 
        The default value is [(self.x_d_[0],self.x_d_[-1])]*N, in which case for each trial the decision boundaries coinside with
        the domain boundaries.
    last_event_is_spike : bool
        If true, trial termination time will not be recorded. Otherwise, trial termination time will be recorded. The default is True. 
...
def _generate_data(self, peq, p0, D, firing_rate_model, num_neuron, boundary_mode, deltaT, time_epoch, decision_bnd, last_event_is_spike):
    ...
    boundary_mode : string
        ...
        Possbile option are "reflecting", "absorbing", None. If set to None, the boundary conditions can be specified
    deltaT : float
        Size of the time bin in seconds for the integration of the Langevin equation. The default is 0.00001.
    time_epoch : list 
        List of N tuples, where N is the number of trials. Each tuple consists of start time and stop time in seconds. For the case of absorbing boundary, stop time will be the maximum allowed time 
    ...
    if p0 is None:
        p0=peq
    x, time_bins, metadata = self._generate_diffusion(peq, p0, D, boundary_mode, deltaT, time_epoch,decision_bnd)
...
def transform_spikes_to_isi(self, spikes, time_epoch, last_event_is_spike, end_of_trial): 
    ...
    time_epoch : list of tuples
         ...
         (in the case of absorbing boundary, trial stop time is a maximum time allowed for a trial, not the actual time of trial termination).
    last_event_is_spike : bool
    ...
    end_of_trial : numpy array (N,), dtype=float
        The actual time of trial termination for each of the trials.
    ...
    for iTrial in range(num_trial):
        ...
        if all_spikes.shape[0] == 0:
            ...
            last_spike_time = time_epoch[iTrial][0] #If no spikes emitted, set to trial beginning time
        else:    
...
def _generate_diffusion(self, peq, p0, D, boundary_mode, deltaT, time_epoch, decision_bnd):
    ...
    decision_bnd : list (N,).
        List that consists of N tuples, each of which consists of the positions of left and right boundaries in the latent space. 
        The default value is [(self.x_d_[0],self.x_d_[-1])]*N, in which case for each trial the decision boundaries coinside with
        the domain boundaries. The deault boundary is considered when None value is provided.
    ...
    bounds = decision_bnd
    for i,boundary in enumerate(decision_bnd):
        if boundary is None:
            bounds[i]=(self.x_d_[0], self.x_d_[-1])    
    ...
    for i,iTrial in enumerate(iter_list):
        ...
        for iBin in range(num_bin):
            ...
            #y[iBin+1] = min( max(y[iBin+1], self.x_d_[0]), self.x_d_[-1])
