...
def _GD_optimization(self,data,save,inference,optimization):
        ...
        data : dictionary
            Training and validation data:
                ...
                dataCV : numpy array (N,2), dtype=np.ndarray. 
                    Validation data in the same format is training data. The default is None.
        save : dictionary 
            Options for saving the results:
                ...
                schedule : numpy array, dtype=int
                    1D array with iteration numbers on which the fitted parameters (e.g. peq and/or D) are saved to RAM/file. 
                    The default is np.arange(1-new_sim, max_iteration+1), which includes all the iteration numbers, and saves fitting results on each iteration. 
                sim_start : int
        ...
        optimization : dictionary
            Optimization options:
                ...
                loglik_tol : float 
                    Threshold for optimization termination due to the lack of likelihood improvement. If relative loglikelihood improvement is less than loglik_tol, stop the
                    optimization. If set to zero, the optimization will not be stopped due to the lack of likelihood imporvement (in this case the optimization
                    will terminate after reaching max_iteration). The default is 0. 
                etaf : float 
                    Regularization strength for the potential function (set to zero for unregularized optimization). The default is 0.               
                gd_type : str
                    Determines optimization strategy for several optimization parameters. Availible options are "coordinate_descent", in which case each of the gradients will be recalculated
                    after each parameter update, or "simultaneous_update", in which case the all of the parameters will be updated on each iteration by a single call to the _get_loglik_data function. 
                    The default is 'simultaneous_update'.
        ...
        results: dictionary 
            Dictionary with the fitted parameters. The possible entries are:
                'loglik': float
                    Training loglikelihood saved on each iteration.
                'loglikCV': float
                    Validation loglikelihood saved on each iteration (only if dataCV is provided).
